...
exports.dbFirstSignUp = async (req, res) => {
  try {
    ...
    if (dBoy?.dbEmail?.isVerify === true && dBoy?.isVerify === true) {
    ...
    await DelBoy.deleteOne({ "dbEmail.email": dbEmail });
...
exports.dbResetPassLinkVerify = async (req, res) => {
  try {
    ...
    const tempToken = crypto
      ...
      .digest("hex");
    const delBoy = await DelBoy.findOne({
      forgotPassToken: tempToken,
      forgotPassExpired: { $gt: Date.now() },
      "dbEmail.isVerify": true,
    });
    if (!delBoy) {
      return res
        .status(400)
        .json({ message: "Invalid Details Or Link Expired" });
    }
...
exports.dbDashCharts = async (req, res) => {
  try {
    ...
    const totalOnlineOrderPercent = picChartPercentage(
      totalOnlineOrder,
      totalOrder
    );
    const totalAcceptOrderPercent = picChartPercentage(
      totalAcceptOrder,
      totalOrder
    );

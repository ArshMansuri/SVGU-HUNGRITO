...
exports.resDashCharts = async (req, res) => {
  try {
    ...
    for (let i = 0; i < orderedDays.length; i++) {
      ...
      let income = 0;
      order.forEach((o) => {
        const resIndex = o?.orders.restu?.findIndex(
          (obj) => obj.resId.toString() === req.restu._id.toString()
        );
        if (resIndex !== -1) {
    ...
    thisYearOrder.forEach((order) => {
      ...
      if (resIndex !== -1) {
        thisYearRevenue[creationDate.getMonth()] +=
          order?.orders?.restu[resIndex]?.resSubTotal;
      }
    });
    ...
    lastYearOrder.forEach((order) => {
      ...
      if (resIndex !== -1) {
        lastYearRevenue[creationDate.getMonth()] +=
          order?.orders?.restu[resIndex]?.resSubTotal;
      }
    });

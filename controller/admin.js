...
exports.getAllNewResList = async (req, res) => {
  try {
    const restus = await Restaurant.find({
      isVerify: false,
      "resFoodImage.publicUrl": {
        $ne: null,
      },
    }).select("resName resCompletAddress.city money");
...
exports.getAllNewDbList = async (req, res) => {
  try {
    const delBoys = await DelBoy.find({
      isVerify: false,
      "dbImage.publicUrl": {
        $ne: null,
      },
    }).select("dbName dbCompletAddress.city money dbImage.publicUrl");
...
exports.getReturnPaymentOrdersList = async (req, res) => {
  try {
    const orders = await Order.find({
      ...
      .populate({ path: "userId", select: "username profilImg phone.phone" })
      .select("OrderTokne orders.total");
...
exports.adminSendNotificationToUser = async (req, res) => {
  try {
    ...
    for (let i = 0; i < users?.length; i++) {
      if (
        users[i]?.notiTokne !== undefined ||
        users[i]?.notiTokne !== "not allow"
      ) {
